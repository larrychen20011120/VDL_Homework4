...
from net.model import PromptIR, PromptIRCLS, NoiseClassifier
...
class PromptIRModel(pl.LightningModule):
    def __init__(self, opt):
        ...
        self.net = PromptIRCLS(decoder=True)
        self.noise_classifier = NoiseClassifier(192, 2)
    ...
    def training_step(self, batch, batch_idx):
        ...
        latent, restored = self.net(degrad_img)

        noise_type = self.noise_classifier(latent)
        
        restored_loss = self.restored_loss_fn(restored, clean_img)
        classification_loss = self.classification_loss_fn(noise_type, label)
        total_loss = torch.mean( restored_loss + self.opt.loss_w * classification_loss ) / self.opt.grad_accumulation
        ...
        self.log("Train/Restoration Loss", restored_loss)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        latent, restored = self.net(degrad_patch)
        ...
        total_loss = torch.mean( restored_loss + self.opt.loss_w * classification_loss )

        psnr, ssim, _ = compute_psnr_ssim(restored, clean_patch)

        pred = torch.argmax(noise_type, dim=1)
        acc = (pred == label).float().mean()

        self.log("Val/Total Loss", total_loss)
        self.log("Val/Classification Loss", classification_loss)
        self.log("Val/Restoration Loss", restored_loss) 
        self.log("Val/Classification Acc", acc)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--loss_w', type=float, default=0.05, help='loss weight of classification')
