...
class PromptIRCLS(nn.Module):
    def __init__(self, 
        ...
        super(PromptIRCLS, self).__init__()
    ...
    def forward(self, inp_img, noise_emb = None):
        ...
        return latent, out_dec_level1
    

class UEM(nn.Module):
    """
    3 × Conv + ReLU → K 張 log σ² map，輸入為復原 RGB 圖 (B,3,H,W)
    """
    def __init__(self, num_tasks: int, mid_ch: int = 32):
        super().__init__()
        self.head = nn.Sequential(
            nn.Conv2d(3,  mid_ch, 3, 1, 1),
            nn.ReLU(inplace=True),
            nn.Conv2d(mid_ch, mid_ch, 3, 1, 1),
            nn.ReLU(inplace=True),
            nn.Conv2d(mid_ch, num_tasks, 3, 1, 1)   # 輸出 log σ²_k
        )
        nn.init.constant_(self.head[-1].bias, 0.)

    def forward(self, img):
        return self.head(img)           # (B,K,H,W) ─ log σ² maps
